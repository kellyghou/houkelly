...
public class ArrayDeque<T> extends AbstractDeque<T> {
    ...
    private void resize(int capacity) {
        ...
        for (int newIndex = 0; newIndex < size; newIndex += 1) {
            newData[newIndex] = data[i];
            i = increment(i, data.length);
            //changed size to data.length to take care of the wrap around
        }
        front = newData.length - 1;
        back = size;
        data = newData;
        ...
        // T[] newData = (T[]) new Object[capacity];
        // int i = increment(front, size);//changed data.length to size
        // for (int newIndex = 0; newIndex < back; newIndex += 1) {
        //     //loops until newIndex < back instead of newIndex < size
        //     newData[newIndex] = data[i];
        //     i = increment(i, size);
        // }
        // front = newData.length - 1;
